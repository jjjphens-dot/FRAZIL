...
|---|---|---|---|---|---|---|---|---|---|
| PluginProcessor | `src/plugin/PluginProcessor.*` | JUCE lifecycle、bus、Host 参数/状态适配、把 block 交给 app | `prepareToPlay`、`processBlock`、state API | JUCE、app | message/audio boundary | `tests/`、CTest、pluginval、plugin integration | 0002, 0003 | `HOST-000`, `HOST-001`, `PARAM-001`, `STATE-001`, `STATE-002`, `AUTO-001` | Implemented M1 foundation; parameter automation audio-path and state/mode integration evidence present; Host/DAW matrix and history boundary integration remain pending; EditHistoryManager remains planned for M5 (HIST-001..004) |
...
| AudioEngine | `src/app/AudioEngine.*` | 处理链生命周期与 gain/global mix 编排；wet path 当前为 pass-through；维护 ProcessSpec buffer invariant | `prepare`、`reset`、`process` | JUCE audio buffer、DSP primitives | audio | `frazil_tests`, `frazil_render`, `frazil_render_cli` | 0003 | `ARCH-001`, `APP-001`, `DSP-001..004` | Implemented M1 skeleton with priming/invariant guard and offline render smoke |
| WaterProcessor | planned `src/dsp/water/` | 双模式、input-driven、source-preserving Water material transform；Fluid=A+B+D，Resonant=C；不拥有 routing/stage/global mix | planned prepare/reset/process + Water engine params；candidate Mode/Size/Motion 经 ParameterMapper 映射 | dsp primitives only | audio | property/render/listening/perf | Proposed 0006 (`ADR-W-001`) | `WATER-001..008` | Planned M2; no production Water DSP or candidate Host controls implemented |
...
| Developer Control Surface | path/interface CANDIDATE；不得由索引预先冻结 | realtime sound exploration、current-parameter control、bounded diagnostics、A/B/reset 和 experiment-config export；不承担 Host acceptance 或 Production UI | narrow parameter interface + future bounded diagnostics/config boundary | plugin parameter interface；non-realtime diagnostics consumer | message/UI；audio thread only publishes bounded data | future DEV-UI interaction/diagnostics/config tests | — | `DEV-UI-001` | PLANNED Water M2 readiness enabler; not implemented |
