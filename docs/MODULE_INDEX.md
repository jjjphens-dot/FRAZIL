...
|---|---|---|---|---|---|---|---|---|---|
| PluginProcessor | `src/plugin/PluginProcessor.*` | JUCE lifecycle、bus、Host 参数/状态适配、把 block 交给 app | `prepareToPlay`、`processBlock`、state API、narrow parameter accessors | JUCE、app | message/audio boundary | `tests/`、CTest、pluginval、plugin integration | 0002, 0003 | `HOST-000`, `HOST-001`, `PARAM-001`, `STATE-001`, `STATE-002`, `AUTO-001` | Implemented M1 foundation; parameter automation audio-path and state/mode integration evidence present; Host/DAW matrix and history boundary integration remain pending; EditHistoryManager remains planned for M5 (HIST-001..004) |
| ParameterContract | `src/app/ParameterContract.h` | 不依赖 JUCE 的 Host-facing range、step、default 和 routing count value contract | shared range/default definitions | app value types only | setup, mapping, state validation, render CLI | parameter regression | 0002 | `PARAM-001` | Implemented M1 |
| ParameterLayout | `src/plugin/ParameterLayout.*` | Host/JUCE-facing 静态 ID、顺序、类型、label、choice、smoothing registration；数值合同来自 `ParameterContract` | layout factory/definitions | JUCE parameter types、ParameterContract | message/setup | parameter regression | 0002 | `PARAM-001` | Implemented M1 |
...
| PluginEditor | `src/plugin/PluginEditor.*` | 当前 M0 占位 editor 与 JUCE lifecycle | editor component | JUCE AudioProcessor interface | message/UI | lifecycle/resize/pluginval | — | `UI-001` | Implemented M0 placeholder |
