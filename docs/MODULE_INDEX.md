...
| AudioEngine | `src/app/AudioEngine.*` | 处理链生命周期与 gain/global mix 编排；wet path 当前为 pass-through | `prepare`、`reset`、`process` | JUCE audio buffer、DSP primitives | audio | `frazil_tests` | 0003 | `ARCH-001`, `APP-001`, `DSP-001..004` | Implemented M1 skeleton |
...
| Smoothing primitives | `src/dsp/primitives/` | 连续参数 smoothing 和离散 transition primitives | `prepare`/`reset`/`setTarget`/`getNextValue` | standard library/value types | audio | step/transition/finite | 0003 | `DSP-004`, `AUTO-001` | Implemented M1 continuous primitive; discrete transition remains M4 |
