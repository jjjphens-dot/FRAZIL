...
| AudioEngine | `src/app/AudioEngine.*` | 处理链生命周期与 gain/global mix 编排；wet path 当前为 pass-through；维护 ProcessSpec buffer invariant | `prepare`、`reset`、`process` | JUCE audio buffer、DSP primitives | audio | `frazil_tests`, `frazil_render` | 0003 | `ARCH-001`, `APP-001`, `DSP-001..004` | Implemented M1 skeleton with priming/invariant guard and offline render smoke |
