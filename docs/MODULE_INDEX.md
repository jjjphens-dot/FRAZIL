...
| RandomSource | planned `src/dsp/primitives/RandomSource.*` | M1 generic、可注入、可复现的 DSP 随机源；M2/M3 使用并补 algorithm-specific semantics | seed/next/reseed value API | standard library only | audio | fixed-seed determinism、instance isolation | future algorithm ADRs only when semantics become production decisions | `DSP-005` | Planned M1 |
