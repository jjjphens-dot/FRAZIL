...
STATE-001 的 versioned StateModel foundation 与 unit/XML transport restore evidence 已随 PR #5 合入 `main`。当前 CTest 已验证 STATE-002 mode-value-retention integration；这不等于真实 DAW save/reopen 已验证，Ableton、FL Studio 和 REAPER 的 save/reopen evidence 仍属于 HOST-001。不得把 `origin/feat/m1-state-contract` 的当前分支状态作为 `main` 的实现事实。
