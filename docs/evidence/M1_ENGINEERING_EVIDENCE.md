...
- `PERF-BASE-001`: Debug, Release, and ASAN reports passed with zero observed `operator new` calls during measured callbacks; all three reports have `configuredCommit=2b931cd6d5bade6630170ab7c0a861a5c68df4ac` and `sourceState=clean`. Each record includes compiler flags, Reference DAW/measurement-tool N/A rationale, thread/instance configuration, statistical method, CPU/denormal observation, and the timing/memory/allocation fields required by `docs/TESTING.md`. The measured values and method are recorded in [PERF-BASE-001.md](PERF-BASE-001.md).
...
- Hosted CI: PR #18 implementation head `2b931cd6d5bade6630170ab7c0a861a5c68df4ac`, Windows Debug / CMake / CTest run `34463783379` completed `SUCCESS`; configure, build, and the remote CTest suite passed 8/8. This is hosted Debug evidence only and does not imply hosted Release/ASAN, pluginval, DAW, or listening evidence.
