...
| Local validation | 当前 branch `94f302f` 的 Debug、Release、ASAN fresh configure、6-job safe build 和 CTest 均通过；portability、Markdown-link、VS Code task scanners 及 regression tests 均 PASS | 已验证；本机绝对路径仅在 ignored local configuration/build output，仓库 preset 保持可移植 |
...
当前 branch `94f302f` 的 TEST-002、PERF-BASE-001 和 ARCH-LAT-001 结果见 [`docs/evidence/M1_ENGINEERING_EVIDENCE.md`](evidence/M1_ENGINEERING_EVIDENCE.md) 与 [`docs/evidence/PERF-BASE-001.md`](evidence/PERF-BASE-001.md)。Debug、Release、ASAN 均通过 fresh configure、safe build 和 8/8 CTest；这批证据只覆盖当前 M1 pass-through/plugin foundation，不提前宣称 Water/Ice/Routing、完整 render、pluginval、真实 DAW 或 M1 Joint Exit 已完成。
