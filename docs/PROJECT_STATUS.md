...
项目处于 **M1 late-stage closure + Water pre-M2 preparation + Developer Sound/Debug Tooling initial slice**。
...
6. `DEV-UI-001` 已有一个仅 Debug/ASAN 可用的初始 Developer Control Surface：它绑定当前 9 个 Host 参数，
   提供 Water experiment-only controls、临时 A/B/reset、draft experiment-config export 和 bounded runtime
   diagnostics；它不是 M5 Production UI，也不改变 Host registry 或 plugin state schema。Dry/Processed comparison、
   完整 DiagnosticsSnapshot/debug bundle workflow、interactive usability acceptance 和 Offline Sound Lab handoff
   仍待完成。Water、Ice、Routing、EditHistoryManager、完整 render regression matrix、离散 enable/routing
   transition 和正式 UI 仍未实现；真实 Host/DAW restore matrix、M5 EditHistoryManager integration、pluginval
   current-artifact validation 和公开兼容性仍待执行；`PARAM-FREEZE-001` 保持为 M2/M3 -> M4 gate，不是 M1 Exit blocker。
...
| UI | Debug/ASAN 中提供初始 DEV-UI-001 surface；Release 保留静态占位界面 | 非产品 UI；Dry/Processed comparison、interactive usability acceptance 和 M5 Production UI 仍未完成 |
| Developer sound tools | RENDER-001、TESTDATA-001、manual performance harness，以及 Debug/ASAN 的 realtime controls、bounded diagnostics 和 draft config export | 初始 DEV-UI-001 slice 已实现；完整 diagnostics/debug bundle、Offline Sound Lab handoff 和 usability workflow 仍为 PLANNED |
...
## 2.7 DEV-UI-001 initial slice validation

2026-09-12 本地工作树 evidence：Debug、Release 和 ASAN 均通过仓库 `tools/vscode_msvc_env.cmd` 初始化
...
本轮还通过 `python tools/check_portability.py` 与 `git diff --check`；未执行当前变更后的 pluginval、真实
DAW/Host matrix、interactive usability review、Dry/Processed listening comparison 或完整 debug bundle。
...
PluginEditor
  ├─ Debug/ASAN: initial DEV-UI-001 controls, temporary A/B/reset and draft config export
...
- 有意保留的未实现项：Water、Ice、Routing、EditHistoryManager、完整 render regression matrix、Dry/Processed comparison、正式 UI 和离散 transition 均仍按 Coding Plan 处于计划阶段；当前 RENDER-001 只覆盖 M1 pass-through offline smoke；本次状态工作不提前创建声音算法或 history 生产依赖。
...
- Developer Sound/Debug Tooling：**initial slice / 进行中**。`DEV-UI-001` 已在 Debug/ASAN 提供当前 9 参数控制、Water experiment-only controls、bounded diagnostics、临时 A/B/reset 和 draft config export；完整 workflow、Dry/Processed comparison、usability acceptance 和 Offline Sound Lab handoff 仍未完成。它是大规模 `EXP-W-002` 前的 Water M2 effective-development-readiness prerequisite，但不是 M1 Exit hard gate。
